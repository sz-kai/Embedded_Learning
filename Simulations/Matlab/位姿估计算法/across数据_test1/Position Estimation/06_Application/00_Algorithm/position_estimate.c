...
void position_estimate_update(Position_Estimate_t *pos_est, __IO Flight_Control_State_t *flgt_ctl)
{
    ...
    for (uint8_t i = 0; i < 3; i++)
    {
        ...
        {
            accel_earth[i] += flgt_ctl->rotation_matrix[i][j] * acc_body[j];
        }
    }
    ...
    if (pos_est->gps_valid)
    {
        ...
        if (_gps_position->eph >= max_eph_epv && _gps_position->epv >= max_eph_epv)
        {
            /*eph和epv越大，GPS精度越低*/
    ...
    else
    {
        ...
        if (_gps_position->eph <= 0.7f * max_eph_epv && _gps_position->epv <= 0.7f * max_eph_epv && _gps_position->satellites_used >= 3)
        {
    ...
    {
        /*转换GPS数据*/
        double lat = _gps_position->lat * 1e-7; /*float只有6-7位有效数字，会损失精度，0.00001 度的差距在地面上大约是 1.1米*/
        double lon = _gps_position->lon * 1e-7;
        double alt = _gps_position->alt * 1e-3;
        /*如果参考位置未设置，则设置参考位置*/
        if (!pos_est->ref_set)
        {
            ...
            if (pos_est->ref_set_count < 10)
            {
                ...
                pos_est->est_x[1] = _gps_position->vel_n_m_s;
                pos_est->est_y[0] = 0.0f;
                pos_est->est_y[1] = _gps_position->vel_e_m_s;
        ...
        else
        {
            ...
            /*速度*/
            gps_corr[0][1] = _gps_position->vel_n_m_s - pos_est->pos_est_buff[est_i][0][1];
            gps_corr[1][1] = _gps_position->vel_e_m_s - pos_est->pos_est_buff[est_i][1][1];
            gps_corr[2][1] = _gps_position->vel_d_m_s - pos_est->pos_est_buff[est_i][2][1];
            /*获取GPS数据对应旋转矩阵*/
            memcpy(rot_mat, pos_est->rot_mat_buff[est_i], sizeof(rot_mat));
            /*GPS修正权值计算，这个权值与基础权值(用于调参)构成最终权值*/
            gps_w_xy = min_eph_epv / MAX(min_eph_epv, _gps_position->eph); /*水平权值自适应，_gps_position->eph越大，GPS数据权值越小*/
            gps_w_z = min_eph_epv / MAX(min_eph_epv, _gps_position->epv);  /*垂直权值自适应，_gps_position->epv越大，GPS数据权值越小*/
        }
    }
    else
    {
    ...
    pos_est->use_gps_xy = pos_est->ref_set && pos_est->gps_valid &&
                          pos_est->param.gps_w_xy_p > MIN_VALID_W && _got_posllh;
    pos_est->use_gps_z = pos_est->ref_set && pos_est->gps_valid &&
                         pos_est->param.gps_w_z_p > MIN_VALID_W && _got_posllh;
    ...
    {
        /*更新epv*/
        pos_est->epv = MIN(pos_est->epv, _gps_position->epv);
    ...
    if (pos_est->can_estimate_xy)
    {
        ...
        {
            /*更新epx*/
            pos_est->eph = MIN(pos_est->eph, _gps_position->eph);
            /*修正水平位置*/
