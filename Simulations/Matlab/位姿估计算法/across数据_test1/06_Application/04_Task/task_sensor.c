...
void Task_Sensor_Init(void)
{
    ...
    ms5611_init(sensor_handle.baro_proc_handle->baro_handle);
...
void Task_BARO(void)
{
    ...
    if (LOOP_FREQ_SET(LOOP_100_Hz, baro_tick, LOOP_500_Hz))
    {
        if (ms5611_read(sensor_handle.baro_proc_handle->baro_handle))
        {
            baro_raw_data.pressure = sensor_handle.baro_proc_handle->baro_handle->last_data.pressure_pa;
            baro_raw_data.temperature = sensor_handle.baro_proc_handle->baro_handle->last_data.temperature_degC;
            baro_processing_update(sensor_handle.baro_proc_handle, &baro_raw_data);
            flgt_ctl.sensor.baro_alt = baro_processing_get_altitude(sensor_handle.baro_proc_handle);
        }
    }
