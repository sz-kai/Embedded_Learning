...
#define ROTATION_SPEED_LIMIT 50.0f
...
void Mahony_Init(Mahony_Param_t *Mhy_P, const Axis3_f_u acc, const Axis3_f_u mag)
{
    Mhy_P->Kp = MAHONY_KP;
    Mhy_P->Ki = MAHONY_KI;
    Mhy_P->integral_x = 0.0f;
    Mhy_P->integral_y = 0.0f;
    Mhy_P->integral_z = 0.0f;
    /*初始欧拉角*/
    float roll = 0.0f;
    float pitch = 0.0f;
    float yaw = 0.0f;
...
#if 1
        /* 2. 计算水平面上的参考磁场强度 (b_x) */
        /* 这一步实际上是在计算这一时刻“理应”存在的北向磁场分量 */
        float b_x = sqrtf(h_e.x * h_e.x + h_e.y * h_e.y);

        /* 3. 计算纯偏航角误差 (err_z) */
        /*
           原理：只比较水平面上的差异。
           在 Code_B 中逻辑是：ez_ef = -(hy * bx);
           这相当于在地理坐标系下，计算测量向量与北向参考向量的叉乘的 Z 分量。
           h_e.y 是测量到的东向分量(理应为0)，b_x 是北向分量。
           结果是一个标量，代表绕地理 Z 轴的旋转误差。
        */
        // float err_z = -(h_e.y * b_x);
        float err_z = -(h_e.y * b_x);

        /* 4. 将地理坐标系的 Z 轴误差投影回机体坐标系 */
        /*
           DCM 是机体到地理的矩阵 (Body -> Earth)。
           我们需要将误差从地理转回机体 (Earth -> Body)，所以乘以 DCM 的转置。
           地理系的 Z 轴向量是 [0, 0, 1]，乘以 DCM 转置后，
           即取 DCM 的第三行 (row 2) 作为投影系数。
