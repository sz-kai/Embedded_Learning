...
float est_z_perd[2] = {0.0f};
float est_x_perd[2] = {0.0f};
float est_y_perd[2] = {0.0f};
 ...
 * @retval 无
 */
void position_estimate_update(Position_Estimate_t *pos_est, volatile Flight_Control_State_t *flgt_ctl)
{
    ...
    {
        /*转换GPS数据*/
        double lat = flgt_ctl->sensor.gps.lat; /*float只有6-7位有效数字，会损失精度，0.00001 度的差距在地面上大约是 1.1米*/
        double lon = flgt_ctl->sensor.gps.lon;
        double alt = flgt_ctl->sensor.gps.alt;
        /*如果参考位置未设置，则设置参考位置*/
        if (!pos_est->ref_set)
        {
    ...
    est_z_perd[0] = pos_est->est_z[0];
    est_z_perd[1] = pos_est->est_z[1];
    /*如果预测值异常，则使用上一时刻的预测值*/
    ...
    if (pos_est->can_estimate_xy)
    {
        ...
        est_x_perd[0] = pos_est->est_x[0];
        est_x_perd[1] = pos_est->est_x[1];
        est_y_perd[0] = pos_est->est_y[0];
        est_y_perd[1] = pos_est->est_y[1];
        /*如果预测值异常，则使用上一时刻的预测值*/
 ...
 * @retval 无
 */
void position_estimate_get_pos_vel(Position_Estimate_t *pos_est, volatile Pos_Vel_t *pos_vel)
{
