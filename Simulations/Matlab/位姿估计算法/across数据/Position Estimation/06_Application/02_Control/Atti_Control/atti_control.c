...
/*姿态角PID控制器参数与状态*/
static PID_Controller_t angle_pid[AXIS_COUNT];
/*角速度PID控制器参数与状态*/
static PID_Controller_t ang_vel_pid[AXIS_COUNT];
...
{
    __IO Axis_State_t *_axis_state = atti_control_ctx->flgt_ctl->control_state;
    __IO Flight_Control_State_t *_flight_state = atti_control_ctx->flgt_ctl;
    /*油门补偿(应在pid计算前进行，可放在别的地方以符合单一职责原则)*/
    /*1.损失系数*/
    float throttle_theta = fabsf(cosf(_axis_state[ROLL].angle) * cosf(_axis_state[PITCH].angle));
    /*2.限幅,0.7相当于单轴倾斜45度，当倾斜角度过大时，会导致油门过大*/
    throttle_theta = throttle_theta < 0.7f ? 0.7f : throttle_theta;
    /*3.油门补偿*/
    _flight_state->throttle = _flight_state->throttle * ((1.0f / throttle_theta - 1.0f) * 0.5f + 1.0f);
    /*4.限幅*/
...
{
    float error;
//    Flight_Control_State_t *_flight_state = atti_control_ctx->flgt_ctl;
    __IO Axis_State_t *_axis_state = atti_control_ctx->flgt_ctl->control_state;
    PID_Controller_t *_angle_pid = atti_control_ctx->angle_pid;
    __IO rc_state_t *_rc_state = atti_control_ctx->rc_state;
    /*PID项*/
    float p_item;
    /*误差计算*/
    /*期望值*/
...
void ang_vel_control(Axis_e axis, atti_control_ctx_t *atti_control_ctx)
{
    ...
    /*误差*/
    error = _axis_state[axis].rate_target * 0.005f - _axis_state[axis].rate; /*注意，这里还需考虑坐标转换*/
    ...
    {
        _ang_vel_pid[axis].i_item = 0;/*这里会不会在飞行过程中被清零？可以只在飞机未解锁时清零*/
    }
    else
    {
...
void atti_control_init(void)
{
    ...
    /*微分项需要初始化上一次测量值*/
    ang_vel_pid[ROLL].last_measurement = flgt_ctl.control_state[ROLL].rate;
    ang_vel_pid[PITCH].last_measurement = flgt_ctl.control_state[PITCH].rate;
    ang_vel_pid[YAW].last_measurement = flgt_ctl.control_state[YAW].rate;
