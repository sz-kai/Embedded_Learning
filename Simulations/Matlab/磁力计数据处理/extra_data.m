...
% %
% % 作者: Kai (基于C代码) / AI
% % 版本: 1.5.0 (适配 MAG_GAUSS, 并更新 MAG_CAL ID)
% % 日期: 2024/05/22
% 
% %% --- 初始化 ---
% clear;
...
% LOG_ID_MAG_RAW    = uint8(0x06);
% %%% 修改: 更新ID并新增ID %%%
% LOG_ID_MAG_GAUSS  = uint8(0x07); % 新增
% LOG_ID_MAG_CAL    = uint8(0x08); % ID从 0x07 改为 0x08
% 
% PACKET_SIZE = 33; % 包的总大小保持不变，因为payload是union
% 
% %% --- 用户选择日志文件 ---
...
%             end
% 
%         %%% 新增: 解析 LOG_ID_MAG_GAUSS 消息 %%%
%         case LOG_ID_MAG_GAUSS
%             % log_mag_gauss_t: 3 * float = 12 bytes
%             if msg_len == 12
%                 clear s;
%                 data = typecast(payload_bytes(1:12), 'single');
%                 s.timestamp = timestamp;
...
% 
%         case LOG_ID_MAG_CAL
%             % log_mag_cal_t: 3 * float = 12 bytes
%             if msg_len == 12
%                 clear s;
%                 data = typecast(payload_bytes(1:12), 'single');
%                 s.timestamp = timestamp;
...
% fprintf('正在生成图表...\n');
% 
% % (原有的 IMU_RAW, IMU_CAL, IMU_FILTER, ATTITUDE 绘图代码保持不变)
% % ...
% 
% % 绘制 IMU 原始数据
...
%     time_s = double(time_ms - time_ms(1)) / 1000.0;
% 
%     roll  = [logData.attitude.roll]  * (180/pi);
%     pitch = [logData.attitude.pitch] * (180/pi);
%     yaw   = [logData.attitude.yaw]   * (180/pi);
% 
...
%     plot(time_s, mag_raw_x, 'r-'); hold on;
%     plot(time_s, mag_raw_y, 'g-');
%     plot(time_s, mag_raw_z, 'b-');
%     title('原始磁力计数据');
%     xlabel('时间 (s)');
%     ylabel('原始值 (ADC Counts)');
%     legend('Mag raw X', 'Mag raw Y', 'Mag raw Z');
%     grid on;
% end
% 
% %%% 新增: 绘制转换后的磁力计数据 (Gauss) %%%
% if ~isempty(logData.mag_gauss)
%     time_ms = [logData.mag_gauss.timestamp];
%     time_s = double(time_ms - time_ms(1)) / 1000.0;
% 
%     mag_gauss_x = [logData.mag_gauss.mag_x];
%     mag_gauss_y = [logData.mag_gauss.mag_y];
%     mag_gauss_z = [logData.mag_gauss.mag_z];
% 
%     figure('Name', 'Magnetometer Gauss Data');
%     plot(time_s, mag_gauss_x, 'r-', 'LineWidth', 1.2); hold on;
%     plot(time_s, mag_gauss_y, 'g-', 'LineWidth', 1.2);
%     plot(time_s, mag_gauss_z, 'b-', 'LineWidth', 1.2);
%     title('转换后的磁力计数据 (高斯)');
%     xlabel('时间 (s)');
%     ylabel('磁场强度 (Gauss)'); % 单位可能是 Gauss 或其他物理单位
%     legend('Mag gauss X', 'Mag gauss Y', 'Mag gauss Z');
%     grid on;
% end
% 
% % 绘制磁力计校准后数据
...
%     plot(time_s, mag_cal_x, 'r-', 'LineWidth', 1.2); hold on;
%     plot(time_s, mag_cal_y, 'g-', 'LineWidth', 1.2);
%     plot(time_s, mag_cal_z, 'b-', 'LineWidth', 1.2);
%     title('校准后的磁力计数据');
%     xlabel('时间 (s)');
%     ylabel('磁场强度 (uT)');
%     legend('Mag cal X', 'Mag cal Y', 'Mag cal Z');
%     grid on;
% end
% 
% % 绘制GPS轨迹
...
if ~isempty(logData.mag_raw) && length(logData.mag_raw) > 10 % 确保有足够的数据点
    ...
    fprintf('拟合椭球半径 (各轴缩放): [%.2f, %.2f, %.2f]\n', radii_raw(1), radii_raw(2), radii_raw(3));
    avg_radius = mean(radii_raw);
    scale_factors = radii_raw / avg_radius;
    ...
    title('原始磁力计数据与椭球拟合', 'FontSize', 14);
...
if ~isempty(logData.mag_gauss) && length(logData.mag_gauss) > 10
    ...
    surf(el_x, el_y, el_z, 'FaceAlpha', 0.2, 'EdgeColor', 'r', 'LineStyle', ':', 'DisplayName', '拟合椭球');
    title('高斯磁力计数据与椭球拟合 (硬磁校准后)', 'FontSize', 14);
    xlabel('X 轴 (Gauss)'); ylabel('Y 轴 (Gauss)'); zlabel('Z 轴 (Gauss)');
    grid on; axis equal;
    legend('show'); view(3);
    hold off;
end
...
if ~isempty(logData.mag_cal) && length(logData.mag_cal) > 10
    ...
    surf(sphere_x*avg_radius_cal, sphere_y*avg_radius_cal, sphere_z*avg_radius_cal, ...
        'FaceAlpha', 0.1, 'EdgeColor', 'k', 'LineStyle', ':', 'DisplayName', '理想球面 (参考)');
    title('校准后磁力计数据与理想球面比较', 'FontSize', 14);
    xlabel('X 轴 (uT)'); ylabel('Y 轴 (uT)'); zlabel('Z 轴 (uT)');
    grid on; axis equal;
    legend('show'); view(3);
    hold off;
end
